...
class Net_UserAgent_Mobile_DoCoMoDisplayMap
{
    ...
    private static $_displayMap = array(
        ...
        if (array_key_exists('DOCOMO_MAP', $_SERVER)
            ...
            do {
                ...
                foreach ($indexes as $modelName => $modelIndexes) {
                    self::$_displayMap[$modelName] = array();
                    foreach ($values[ $modelIndexes[0] ]['attributes'] as $attributeName => $attributeValue) {
                        self::$_displayMap[$modelName][ strtolower($attributeName) ] = $attributeValue;
                    }
                }

                self::$_xmlParsed = true;
            } while (false);
        }

        return @self::$_displayMap[ strtoupper($model) ];
    }
